...
def test_case(dataset,folder_to_stock_project, file_info_processing):
    ...
    file_info_processing_cs = file_info_processing + "_" + project_name + ".txt"
    info_file = open(file_info_processing_cs, "a")
    info_file.write("************************\n")
    ...
    if gcps_path != "N":
        gcpsFile = gcps_path.split("++")[0]
        gcpsCS = gcps_path.split("++")[1]
        project.info(gcpsCS)
        project.info(gcpsFile)
        cluster.add_gcps_from(gcpsFile, epsg_code=int(gcpsCS)).wait()
    ...
    project.info(str(reconstruction.metrics.calibrated_cameras_count))
